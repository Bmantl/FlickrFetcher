...
@interface FlickrURLMaker()

@property (nonatomic, strong) NSString *APIKey;

@end

@implementation FlickrURLMaker
...
- (NSURL *)URLForPhotosInPlace:(FlickrPlace *)flickPlace maxResults:(NSUInteger)maxResults {
  return [self URLForQuery:[NSString stringWithFormat:@"https://api.flickr.com/services/rest/?method=flickr.photos.search&place_id=%@&per_page=%ld&extras=original_format,tags,description,geo,date_upload,owner_name,place_url", [flickPlace placeId], maxResults]];
}

- (NSURL *)URLForPhoto:(FlickrPhoto *)flickrPhoto format:(FlickrPhotoFormat)format
{
  return [NSURL URLWithString:[FlickrURLMaker urlStringForPhoto:flickrPhoto format:format]];
}
...
{
  query = [NSString
           stringWithFormat:@"%@&format=json&nojsoncallback=1&api_key=%@", query, self.APIKey];
  query = [query stringByAddingPercentEscapesUsingEncoding:NSUTF8StringEncoding];
  return [NSURL URLWithString:query];
}

+ (NSString *)urlStringForPhoto:(FlickrPhoto *)flickrPhoto format:(FlickrPhotoFormat)format
{
